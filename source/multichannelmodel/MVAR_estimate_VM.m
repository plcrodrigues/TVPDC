function [ap, efp, pfp] = MVAR_estimate_VM(x,p)
%   ap = MVAR_estimate_VM(x,p)
%
%   Use the Vieira-Morf algorithm for the MVAR estimation. The algorithm
...
%                          example: ap = [I A1 A2 A3] 
    ...
    % estima a matriz Lambda (usando a ideia de Vieira-Morf)
    Lambda(:,:,1) = estimateLambdaMatrix(Pf0_est, Pb0_est, Pfb0_est);

    % calcula as matrizes de coeficientes de reflexao forward e backward
    RCmf = (chol(Pf0,'lower'))*Lambda(:,:,1)*(chol(Pb0,'lower')\eye(m));
    RCmb = (chol(Pb0,'lower'))*Lambda(:,:,1)'*(chol(Pf0,'lower')\eye(m));
    ...
    for k = 2:p  
        ...
        Lambda(:,:,k) = estimateLambdaMatrix(Pf_est, Pb_est, Pfb_est);

        % calcula as matrizes de coeficientes de reflexao forward e backward          
        RCmf = (chol(Pf(:,:,k-1),'lower'))*Lambda(:,:,k)*(chol(Pb(:,:,k-1),'lower')\eye(m));
        RCmb = (chol(Pb(:,:,k-1),'lower'))*Lambda(:,:,k)'*(chol(Pf(:,:,k-1),'lower')\eye(m));
...
function Lambda = estimateLambdaMatrix(Pf_est, Pb_est, Pfb_est)

    m = size(Pf_est,1);
    A = (chol(Pf_est,'lower'))\eye(m);
    B = Pfb_est;
    C = (chol(Pb_est,'lower'))\eye(m);
    Lambda = A*B*C';
