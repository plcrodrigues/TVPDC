...
function results = gpdc_tf_bias(x,sz_window,p,nf,ntrials,type)
%% results = gpdc_tf_bias(x,sz_window,p,nf,ntrials,type)
%
%  INPUTS:
%
...
%
    ...
    if (strcmp(type,'ns') || strcmp(type,'vm') || strcmp(type,'ls'))
        ...
        for w = 1:nb_window
            ...
            if (strcmp(type,'ns'))
                [ap_est, ef, pf]  = MVAR_estimate_NS(reshape(x_window_normalized,m,[]),p);
            elseif (strcmp(type,'vm'))
                [ap_est, ef, pf]  = MVAR_estimate_VM(reshape(x_window_normalized,m,[]),p);
            elseif (strcmp(type,'ls'))
                [ap_est,pf] = MVAR_estimate_LS(reshape(x_window_normalized,m,[]),p);     
                ef = [];
            end
    ...
    elseif strcmp(type,'rls')    
                  ...
                  c = 0.2;
         [apn, pfn] = MVAR_rls(reshape(x_normalized,m,[]),p,c);
