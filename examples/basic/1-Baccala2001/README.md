...
## Description
In this folder you have a `generatefigures.m` script for generating some of the figures in the referenced article. I did not reproduce all of them, only the ones I thought would best show the power of causality analysis via PDC.
...
It should be noted too that the estimations in `generatefigures.m` were all done to illustrate the PDC, so concerns such as model order via Akaike's Information Criterion were not taken into consideration. For simplicity, the order of the estimated models is exactly the order of the MVAR process which generated the samples.
...
<div style="text-align: center;"><img src="./graph_figure1.svg" align="middle" width="180"></div>
<br>

The results of the PDC estimation from a simulation of ${\bf x}(n)$ are obtained with `generatefigures.m`. The graphs below display the absolute value of the PDC between each pair of channels.
...
Note that in this case we have some interesting patterns, such as a **loop** between channels 4 and 5 (they form an oscillator), and an **indirect connection** between channel 1 and channel 5, which the PDC should not give a non-zero value. Let's see what results we get with the `generatefigures.m` script.

<div style="text-align: center;"><img src="./figure2.svg" align="middle" width="640"></div>
...
<div style="text-align: center;"><img src="./figure3.svg" align="middle" width="640"></div>
